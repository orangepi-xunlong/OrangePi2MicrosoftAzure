...
#include "../../utils.h"
#include <OrangePi/v4l2op.h> // this header will includes the ../../mjpg_streamer.h
#include <OrangePi/huffman.h>
#include <OrangePi/jpeg_utils.h>
#include <OrangePi/dynctrl.h>

#define INPUT_PLUGIN_NAME "OrangePi webcam grabber"
...
int input_init(input_parameter *param, int id)
{
    ...
    int width = 640, height = 480, fps = 5, format = V4L2_PIX_FMT_YUV420, i;
    ...
    IPRINT("Format............: %s\n", (format == V4L2_PIX_FMT_YUV420) ? "YUV420" : "MJPEG");
    if(format == V4L2_PIX_FMT_YUV420)
        IPRINT("JPEG Quality......: %d\n", gquality);

    DBG("vdIn pn: %d\n", id);
    /* open video device and prepare data structure */
    if(OrangePi_VideoIn(cams[id].videoIn, dev, width, height, fps, format, 1, cams[id].pglobal, id) < 0) {
...
void *cam_thread(void *arg)
{
    ...
    while(!pglobal->stop) {
        ...
        /* grab a frame */
        if(OrangePi_Grab(pcontext->videoIn) < 0) {
         ...
         */
        if(pcontext->videoIn->formatIn == V4L2_PIX_FMT_YUV420) {
